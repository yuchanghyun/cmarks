...
<p align="center">
  <picture>
    ...
    <img src="docs/images/light.png" alt="파일 트리, 아웃라인, 탭 두 개, 분할 패인이 있는 cmarks 워크스페이스" width="960">
  </picture>
</p>
...
- **워크스페이스·탭·분할·창**: 폴더 단위 워크스페이스, 패인마다 탭, 오른쪽·아래 분할, 드래그로 크기 조정, 패인 확대. 앱을 다시 켜면 그대로 복원됩니다. 단축키 배치는 [cmux](https://github.com/manaflow-ai/cmux)와 같습니다.
- **사이드바**: 파일 트리(파일 시스템 변경을 자동 반영), 문서 아웃라인(현재 위치 강조), 빠른 열기(⌘P, 흐린 검색).
- **워크스페이스에서 찾기(⌘⇧F)**: 워크스페이스의 모든 마크다운 파일 내용을 찾아 파일·줄·문맥을 보여 주고, 고르면 그 위치로 엽니다.
- **라이브 리로드**: 파일을 저장하면 바뀐 부분만 제자리에서 갱신됩니다. 삭제·이동을 감지하고 파일이 돌아오면 자동 복구합니다.

  <img src="docs/images/live-reload.gif" alt="다른 앱이 파일을 고치는 동안 할 일 목록이 제자리에서 갱신되는 모습" width="960">
...
- **Quick Look**: Finder에서 마크다운 파일을 선택하고 스페이스바를 누르면 앱과 같은 모양으로 보입니다(하이라이팅, 수식, 알림, 태스크 리스트, 이모지, 문서 옆 이미지). Mermaid 다이어그램은 Quick Look에서는 코드로 보입니다.
- **Finder 선택 따라가기**: 보기 메뉴에서 켜 두면 Finder에서 고르는 마크다운 파일을 cmarks가 그때그때 보여 줍니다. 사라지지 않는 Quick Look입니다.
- **사용자 CSS**(설정 ▸ 외형)와 파일별 읽던 자리 기억.
- **연동**: Finder "다음으로 열기", 드래그&드롭, `cmarks` 명령줄 도구, `cmarks://open?path=` 딥링크, 파일 ▸ 최근 파일 열기. 설치 시 `.md` 기본 앱으로 지정할 수 있습니다.
...
| 동작 | 단축키 |
|---|---|
| 새 워크스페이스 · 새 창 · 워크스페이스 1–8 · 마지막 | ⌘N · ⌥⌘N · ⌘1–8 · ⌘9 |
...
- 창 사이 탭 드래그는 아직 지원하지 않습니다.
- Quick Look에서 Mermaid 다이어그램은 코드로 보입니다. QLMarkdown 같은 다른 마크다운 Quick Look 확장이 있으면 macOS는 그중 하나만 쓰므로 시스템 설정 ▸ 일반 ▸ 로그인 항목 및 확장 프로그램 ▸ Quick Look에서 고르세요.
